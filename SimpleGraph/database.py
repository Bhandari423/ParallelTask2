...
def add_node(data, identifier=None):
    def _add_node(cursor):
        cursor.execute("INSERT OR REPLACE INTO nodes VALUES(json(?))", (json.dumps(_set_id(identifier, data)),))
    return _add_node
...
def connect_nodes(source_id, target_id, properties={}):
    def _connect_nodes(cursor):
        cursor.execute("INSERT OR REPLACE INTO edges VALUES(?, ?, json(?))", (source_id, target_id, json.dumps(properties),))
